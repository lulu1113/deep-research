...
| 10 | **目录仅大章节** | 目录仅含顶层章节（一、二、三...），不含子标题 |
...
☐ 扫描所有 `##` 标题，仅保留顶层大章节（不含小节/子标题）
  - 识别规则：编号序列首轮出现的 `一、二、三...` 为大章节，后续重置的编号序列为子章节，不进入目录
  - 大章节若无编号，自动分配中文编号（一、二、三...）
  - 编号超过二十时改用阿拉伯数字（保持阅读友好）
☐ 在报告正文前生成目录（仅含大章节，`- [标题](#锚点)` 可点击格式）
