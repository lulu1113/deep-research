...
☐ **dr_tools.py qa-report 通过**（编码/乱码/标题/元数据/TOC/尾部/年份/字数全部检查）
☐ 章节完整性：所有章节存在
☐ 行数：wc -l ≥ 模式参考值
☐ 段落数：抽 2 章，每章 ≥ 5 段
...
字数超标 → 用 `word-count` 获取精确字数，对比全文上限（quick 10,000 / standard 16,000 / deep 28,000）。超标 ≤ 10% → 加声明继续。超标 > 10% → 定位字数最多的 1 章，对其章节 agent 发起 1 次补刀（局部精简），最多 1 轮，不循环。
