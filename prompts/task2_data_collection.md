...
Step 1 — 所有子问题 Exa 搜索并行执行
   ...
   **Exa 不可用检测**：如果所有子问题的 Exa 搜索均返回 rate limit 错误或空结果（无有效 URL），则标记 `exa_unavailable=true`。
   当 `exa_unavailable=true` 时，Step 1 直接结束（不补搜），转入 Step 1.5 作为主搜索路径。

Step 1.5 — 免费数据源补强（与 Step 2 并行）
   触发条件：Exa 返回的可用 URL < 3 或全部年份早于 target_year-2，或 `exa_unavailable=true`。
   当 `exa_unavailable=true` 时，**强制执行**补强，限时从 30s 延长至 **60s**。
   ...
   **禁止**：在 Step 1.5 内部调用 explore agent 或其他子 agent 做搜索——搜索引擎直连更快。
   ...
   - ⏱ 总运行时间 > 30 秒（`exa_unavailable=true` 时 > 60 秒）
...
5. 矛盾数据并排记录，不得合并
6. **补强限时**：Step 1.5 从启动到终止总耗时默认 ≤ 30 秒（`exa_unavailable=true` 时延长至 ≤ 60 秒）。超时后已获得的 URL 继续进入 Step 2，未完成的子问题标记 gap。不得因为补强未完成阻塞 Step 2 抓取或数据池构建。
